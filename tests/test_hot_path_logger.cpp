...
constexpr uint32_t kTrueSize = ObserverData::message_size - 1;

TEST(HotPathLogger, MessageLength) {
    EXPECT_EQ(kTrueSize, 118);
...
TEST(HotPathLogger, StringLiterals) {
    ...
    ObserverData result;
    {
        ...
        EXPECT_EQ(buffer.Read(result), ReadResult::kBufferIsEmpty);

        HOT_INFO << "bar" << Endl;
        EXPECT_EQ(buffer.Read(result), ReadResult::kSuccess);
        EXPECT_EQ(std::string(result.message), std::string("bar"));
    }
    {
        std::string str = test::GenerateRandomString(kTrueSize);
...
TEST(HotPathLogger, Integers) {
    ...
    {
        std::string str = test::GenerateRandomString(kTrueSize - 2);
...
TEST(HotPathLogger, Doubles) {
    ...
    {
        std::string str = test::GenerateRandomString(kTrueSize - 5);
